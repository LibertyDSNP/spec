...
| `issuer` | YES <sup>1</sup> | String | MUST be a [DSNP DID](./DID.md) or a DID with a reciprocal `alsoKnownAs` relationship with a DSNP DID  |
...
<sup>1</sup> The `issuer` field is required, even for credential schema documents that do not themselves include attestation.
...
A DSNP application MAY use the indicated values as a hint when displaying information about the subject of a Verified Credential utilizing this schema.
