...
| `issuer` | YES | String | DID of credential issuer | MUST be a [DSNP DID](./DID.md)<sup>1</sup> |
...
<sup>1</sup> The `issuer` field is required, even for credentials that do not include a `proof`.
...
| `cryptosuite` | YES | String | Cryptographic algorithm identifier | MUST be `eddsa-2022` |
