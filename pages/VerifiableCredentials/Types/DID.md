...
Applications that make use of Verification Credentials issued by DSNP Users MUST be able to resolve DSNP User DIDs to DID documents in order to verify that signatures were created with keys controlled by that user.
...
|
| `service` | NO | Array | Set of DSNP services associated with this user (see below) |
...
The optional `service` array MAY contain the following DSNP Services.

#### Attestation Service Type

The `DSNPAttestation` service type links to a Verifiable Credential that the user has associated with their DSNP User Id via the `attestation` User Data type.
Including certain attestations within the DID document enables applications to reliably verify chains of trust specified as within DSNP [Verifiable Credential Schema](./VerifiableCredentialSchema.md) documents.

The service object has the following fields:

| Property | Required | JSON Type | Description | Restrictions |
| --- | --- | --- | --- | --- |
| `id` | YES | String | The full URI of this service | MUST be of the form `did:dsnp:{userId}#{identifier}`; MUST be unique within the document |
| `type` | YES | String | The type of this service | MUST be `DSNPAttributeSet` |
| `attributeSetType` | YES | String | DSNP Attribute Set Type of the document at `serviceEndpoint` |
| `serviceEndpoint` | YES | String | The URL of a DSNP [Verifiable Credential](./VerifiableCredential.md) | MUST use the `https` scheme |
| `hash` | YES | Array of strings | Multibase-encoded multihash values of content at `serviceEndpoint`; see requirements at [Hash](../../ActivityContent/Associated/Hash.md) |

A DID document MAY include more than one `DSNPAttributeSet` service for a given Attribute Set Type.
...
```json
{
  ...
  "service": [
    {
      "id": "did:dsnp:13972#service1",
      "type": "DSNPAttestation",
      "attributeSetType": "did:dsnp:123456#CarDealership",
      "serviceEndpoint": "https://mysite.com/certifications/authorized_dealer.json",
      "hash": [
